...
Cypress.Commands.add("inputTextWithXpath", (locator, input) => {
  cy.waitUntil(() => cy.xpath(locator).should("not.be.disabled").type(input));
});
...
Cypress.Commands.add("xpathWithClickFirst", text => {
  const xpathLocator = `//*[text()='${text}']`;
  ...
  cy.xpath(xpathLocator).first().click({ force: true });
});

Cypress.Commands.add("xpathWithClickLast", text => {
  const xpathLocator = `//*[text()='${text}']`;
  ...
  cy.xpath(xpathLocator).last().click({ force: true });
});
...
Cypress.Commands.add("mouseOverWithXpath", text => {
  cy.xpath(`//*[text()='${text}']`).trigger("mouseover");
});
Cypress.Commands.add("interceptDoc", endpoint => {
...
Cypress.Commands.add("getAPI", (url, headers, qs) => {
  cy.request({
    method: "GET",
    url: url,
    headers: headers,
...
Cypress.Commands.add("postAPI", (url, headers, body) => {
  cy.request({
    method: "POST",
    url: url,
    headers: headers,
