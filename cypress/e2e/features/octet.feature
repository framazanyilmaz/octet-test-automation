...
Feature: Verify succesfuly payment
  Scenario: Should payment via banks's pos
   ...
   When Check pos connection info "searchBankName","bankTransaction"
   ...
   When Type text with xpath on the "posName","Test-POS11"
   ...
   When Type text with xpath on the "apiUserName","FRY11-TestAPI"
   When Type text with xpath on the "apiPassword","FRY1"
   When Type text with xpath on the "threeDKey","82"
   When Type text with xpath on the "posNo","08539593"
   ...
   When Get "Test-POS11" transaction button using index "posListTable"
   When Click transaction button with index
   Then Verify on the text "Komisyonlar" 
   When Click button with text "Komisyonlar"
   When Click button with text "Ekle"
   When Click button for tomorrow date "startDate","todayButton"
   ...
   When Click button with text "Test-POS11"
